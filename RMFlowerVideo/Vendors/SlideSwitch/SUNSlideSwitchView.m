...
//    _topScrollView.showsVerticalScrollIndicator = NO;
    ...
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView, [UIScreen mainScreen].bounds.size.width, self.bounds.size.height - kHeightOfTopScrollView)];
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        if(self.isAmplificationBtn){
            NSDictionary *attribute = @{NSFontAttributeName: FONT(kFontSizeOfTabButton)};
            CGSize textSize = [vc.title boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) options:\
            ...
            //设置按钮尺寸
            int value = 0;
            if ([self.viewControllerIdentify isEqualToString:@"排行榜"]){
                value = 19;
            }
            
            UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(0, 42, [UIScreen mainScreen].bounds.size.width, 2)];
            line.backgroundColor = [UIColor colorWithRed:0.91 green:0.33 blue:0.22 alpha:1];
            [self addSubview:line];
            float width = [UIScreen mainScreen].bounds.size.width/[_viewArray count];
//            [button setFrame:CGRectMake(xOffset+value, 0, textSize.width, kHeightOfTopScrollView)];
            [button setFrame:CGRectMake(i*width, 0, width, kHeightOfTopScrollView)];
            //计算下一个tab的x偏移量
            xOffset += textSize.width + kWidthOfButtonMargin+value + 24;

            [button setTag:i+100];
            if (i == 0) {
                _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, -20, textSize.width, _shadowImage.size.height);
                button.selected = YES;
                button.titleLabel.font = FONT(kFontSizeOfTabButton+1);//[UIFont systemFontOfSize:kFontSizeOfTabButton+1];
//                [button setFrame:CGRectMake(button.frame.origin.x, button.frame.origin.y-3, button.frame.size.width+5, button.frame.size.height+5)];
            }else{
                button.titleLabel.font = FONT(kFontSizeOfTabButton);
            }
            ...
            button.titleLabel.font = FONT(kFontSizeOfTabButton);
            [button setFrame:CGRectMake(i*(ScreenWidth/2), 0, ScreenWidth/2-2, kHeightOfTopScrollView)];
            if (i == 0) {
                button.selected = YES;
            }
            [button setTag:i+100];
    ...
    //设置顶部滚动视图的内容总尺寸
//    _topScrollView.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, kHeightOfTopScrollView);
}
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (sender.tag != _userSelectedChannelID) {
        ...
        lastButton.titleLabel.font = FONT(kFontSizeOfTabButton-1);
        [lastButton setFrame:CGRectMake(lastButton.frame.origin.x, lastButton.frame.origin.y, lastButton.frame.size.width, lastButton.frame.size.height)];
        //赋值按钮ID
    ...
    if (!sender.selected) {
        sender.selected = YES;
        sender.titleLabel.font = FONT(kFontSizeOfTabButton+1);
        [sender setFrame:CGRectMake(sender.frame.origin.x, sender.frame.origin.y, sender.frame.size.width, sender.frame.size.height)];
        [UIView animateWithDuration:0.25 animations:^{
            
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, -20, sender.frame.size.width, _shadowImage.size.height)];
